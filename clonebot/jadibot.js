...
const jadibot = async (ghost, m, from) => {
      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return ghost.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
      ...
      client[from].ev.process(async (events) => {
        if (events['messages.upsert']) {
          ...
          for (let msg of upsert.messages) {
            ...
            require("../anyone.js")(client[from], m, store)
          }
        }
      })
...
async function stopjadibot(ghost, from) {
  if (!Object.keys(client).includes(from)) {
    return ghost.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
...
async function listjadibot(ghost, m) {
  ...
  return ghost.sendMessage(from, { text: text.trim(), mentions, })
}
