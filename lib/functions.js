...
example = (isPrefix, command, args) => {
  return `• ${texted('bold', 'Example')} : ${isPrefix + command} ${args}`
}
    ...
    try {
      if (Buffer.isBuffer(source)) {
        ...
        axios.get(source, {
          ...
          let file = fs.createWriteStream(`temp/${uuid() + '.' + extension}`)
          ...
          file.on('finish', async () => {
            let data = {
              ...
              extension: extension,
              size: formatSize(response.headers['content-length'] ? response.headers['content-length'] : 0),
